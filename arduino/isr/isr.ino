...
int r = 1;
volatile float wakeupinterval = 1;//1831  // wakeupinterval =  time_in_seconds/((256*1024)/ 8000000))
...
void raspiGoodmorning()
  {    
      ...
      if (r==6) //ask if first time wakeup
      {
       ...
       if (r>6) //setting timer
      {      
        minutes = (float)r;
        time_in_seconds = minutes*60.0;
        wakeupinterval = time_in_seconds/0.032768;//((256.*1024)/ 8000000)
        count = 0;
        Serial.print("setting timer and the wakeupinterval= "); 
        Serial.println(wakeupinterval,DEC);   
      }
